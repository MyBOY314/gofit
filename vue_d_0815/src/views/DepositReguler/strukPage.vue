<template>
    <div class="card">
      <div class="card-body">
        ...
        <h5 class="card-title">Struk Deposit Reguler</h5>
        <div ref="pdfContent">
          <p v-for="(depositReguler, id) in depositReguler" :key="id">
            ...
            <strong>Nama Member: {{ depositReguler.member.nama_member}}</strong> <br>
            <strong>Kasir: {{ depositReguler.pegawai.nama }}</strong><br>
          </p>
        </div>
  ...
  export default {
    setup() {
      ...
      const pegawai = reactive({ data: [] })
      const member = reactive({ data: [] })
      const router = useRouter();
      const route = useRoute();
  
      const generatePDF = () => {
        if (depositReguler.length > 0) {
          const depositReguler = depositReguler[0];
          const doc = new jsPDF();
          ...
          const tableData = [
            ...
            ['Member', depositReguler.member.nama_member],
            ['Kasir', depositReguler.pegawai.nama],
          ];
          doc.autoTable({
            startY: 30,
            head: [['', '']],
...
let pegawai = ref([])
        ...
        axios.get(`http://localhost:8000/api/depositReguler/${route.params.id}`)
          .then((response) => {
            depositReguler.push(response.data.data);
          })
          .catch((error) => {
            console.log(error.response.data);
          });
